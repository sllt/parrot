...
import (
	...
	"github.com/sllt/parrot/internal/pkg/helper"
...
func init() {
	CmdRun.Flags().StringVarP(&excludeDir, "excludeDir", "", excludeDir, `eg: parrot run --excludeDir="tmp,vendor,.git,.idea"`)
	CmdRun.Flags().StringVarP(&includeExt, "includeExt", "", includeExt, `eg: parrot run --includeExt="go,tpl,tmpl,html,yaml,yml,toml,ini,json"`)
	if excludeDir == "" {
...
var CmdRun = &cobra.Command{
	Use:     "run",
	Short:   "parrot run [main.go path]",
	Long:    "parrot run [main.go path]",
	Example: "parrot run cmd/server",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("\033[35mparrot run %s.\033[0m\n", dir)
